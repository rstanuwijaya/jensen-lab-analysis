...
class time_bins:
    ...
    cmap = None
    coincidence_count = dict()
    pixel_coincidence_count = dict()
    ...
    def initialize_pixel_all_coincidence_count(self, coincidence_window = 2, spot = ['all']):
        '''
        param: coincidence_window
        Initialize pixel self coincidence count
    ...
    def initialize_pixel_coincidence_count(self, Fmat, coincidence_window):
        if self.debug: print(self.name, 'Initializing pixel_coincidence_count')
        consts = self.consts
        height = self.consts['number_of_pixels'][0]
        width = self.consts['number_of_pixels'][1]
        num_px = height * width
        total_coincidence_count = np.zeros(num_px, dtype=int)
        for i in range(consts['number_of_frames']):
            time_bin = self.adjusted_bins[i].flatten()
            detection_arg = time_bin.nonzero()
            detection_time_bin = time_bin[detection_arg]
            Fmat_detection = Fmat[detection_arg].T[detection_arg]
            r1 = np.tile(detection_time_bin, detection_time_bin.shape[0])
            r2 = np.repeat(detection_time_bin, detection_time_bin.shape[0])
            diff = (r1-r2).reshape(detection_time_bin.shape[0], detection_time_bin.shape[0])
            coincidence_detection_arg = np.abs(diff) <= coincidence_window
            coincidence_detection_arg = coincidence_detection_arg & Fmat_detection
            coincidence_detection_count = np.sum(coincidence_detection_arg, axis=1)
            coincidence_count = np.zeros(height*width, dtype=int)
            coincidence_count[detection_arg] += coincidence_detection_count
            total_coincidence_count += coincidence_count
        self.pixel_coincidence_count = total_coincidence_count.reshape((height, width))
